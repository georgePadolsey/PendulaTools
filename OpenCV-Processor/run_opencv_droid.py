...
def do_vid(CUR_VID, resize=None):
    ...
    def with_colour_mask(frame, color_low, color_high):
        ...
        return cv2.inRange(frame, color_low, color_high)
    ...
    while cap.isOpened():
        ...
        if resize is not None:
            crop_frame = imutils.resize(crop_frame, resize)
        ...
        if len(y_cnts) > 0:
            ...
            if sq_cross != 0:
                # m per pixel
                last_few_ratios.append(np.sqrt(AC_CROSS_SQ/sq_cross))

                # m per pixel

                cur_ratio = np.min(last_few_ratios)
        ...
        if len(g_cnts) > 0:
            ...
            if has_red and len(r_cnts) > 0:
                cs = sorted(r_cnts, key=cv2.contourArea, reverse=True)
                for c in cs:
                    ((x, y), radius) = cv2.minEnclosingCircle(c)
                    M = cv2.moments(c)
        ...
        data_pts.append({"frame_i": frame_i, "time": time, "m_per_pixel": cur_ratio, "g_center": to_norm(g_adjusted_center),
                         "r_center": to_norm(r_adjusted_center), "rect_points": to_norm(rect_points.flatten()), "angle": angle})

        if frame_i % int(fps) == 0:
            if FAST_MODE:
